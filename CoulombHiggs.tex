...
\bOm(\gamma,\zeta,y,t) 
  =& %\sum_\ell 
  \sum_{\substack{\{\vec M^{(i)}\} \\
  \sum_{i=1}^\ell \vec M^{(i)}=\vec N \\  \vec M^{(i)}\parallel \vec N
...
\mathematica{1.0}{ <<CoulombHiggs`}{CoulombHiggs 7.0 - A package for evaluating quiver invariants. }
...
\mathematica{1.0}{SetDirectory[NotebookDirectory[]]; <<CoulombHiggs`}{CoulombHiggs 7.0- A package for evaluating quiver invariants.}
...
For computing D6 or D4 framed unrefined NCDT invariants for the conifold up to order $x^5$, one may use

\mathematica{1.0}{
\{Tiling, Fan, hMat, Wp, Wm, v1, v2\} = BraneTilingsData[[2]];
UnrefinedSeriesFromCrystal[hMat, D6Framing[hMat, 1], 5]/.y->1}
   {
   $1 + x_1 - 2 x_1 x_2 - 4 x_1^2 x_2 - 2 x_1^3 x_2 +  x_1 x_2^2 + 8 x_1^2 x_2^2 + 14 x_1^3 x_2^2 - 4 x_1^2 x_2^3$
   }
  
\mathematica{1.0}{
UnrefinedSeriesFromCrystal[hMat, D4Framing[hMat, 1, 2, 1], 5]/.y->1}
   {
   $1 + x_1 + 2 x_1 x_2 + 4 x_1^2 x_2 + 2 x_1^3 x_2 +  x_1 x_2^2 + 8 x_1^2 x_2^2 + 14 x_1^3 x_2^2 + 4 x_1^2 x_2^3$
   }
Setting $y=-1$ instead gives the number of molten crystals
at each order. For computing the refined version of these invariants, one may use

\mathematica{1.0}{
D6FramedNCDTSeries[hMat, Wp, Wm, 1, 5] }
   {
   $1 + x_1 + (-y - y^3) x_1 x_2 + (-1/y - 2 y - y^3) x_1^2 x_2 + (-1/y - y) x_1^3 x_2 +  x_1 x_2^2 + (2 + 3 y^2 + 2 y^4 +     y^6) x_1^2 x_2^2 + (3 + 1/y^2 + 6 y^2 +     3 y^4 + y^6) x_1^3 x_2^2 + (-1/y - 2 y - y^3) x_1^2 x_2^3$
   }
  
\mathematica{1.0}{
D4FramedNCDTSeries[hMat, Wp, Wm, 1, 2, 1, 5] }
   {
   $1 + x_1 + (1/y^2 + y^2) x_1 x_2 + (2 + 1/y^2 + y^2) x_1^2 x_2 + 2 
x_1^3 x_2 +  x_1 x_2^2 + (2 + 1/y^4 + 2/y^2 +     2 y^2 + y^4) x_1^2 x_2^2 + (4 + 1/y^4 + 4/y^2 +     4 y^2 + y^4) x_1^3 x_2^2 + (2 + 1/y^2 + y^2) x_1^2 x_2^3$
   }
...
Denef equations. Version 7.0 implements formulas from \cite{Descombes:2021snc} to compute directly D4 and D6 framed refined DT invariants from quivers with potential.\footnote{The routines and functions in this version were implemented by Léo Buisine.} 
...
a matrix of weights $h(p_i)$  of arrows $p_i^\beta:i\to \infty$. The result is a generating series 
...
\defn{NCDTSeriesFromCrystal}{\vardef{hMat}, \vardef{fMat}, \vardef{weight}\footnote{For backward compatibility with version 6 and below, the argument $\var{weight}$ can be replaced by two arguments \var{theta}, \var{phi} defining the angle of the action, such that $\var{weight} = \cos(\theta)(\cos(\phi)h_1 + \sin(\phi)h_2) + \sin(\theta)h_3$.}, \vardef{Nmax}}{constructs the generating function of (localized) refined NCDT invariants for the quiver with height matrix $\var{hMat}$ and framing data $\var{fMat}$ using the refined Quiver Yangian algorithm, for dimension vectors with height up to $\var{NMax}$. The refinement comes from the localization under the $\mathbb C^\times$ action given by $\var{weight} = w_1 h_1 + w_2 h_2 + w_3 h_3$, chosen inside the canonical $(\mathbb C^\times)^3$ action of the toric variety. If $w_3=0$, the action preserves the superpotential. $\var{fMat}$ is a list of two elements: the first is
a matrix of weights $h(q_i^\alpha)$  of arrows $q_i^\alpha:\infty\to i$, and the second is  
a matrix of weights $h(p_i)$  of arrows $p_i^\beta:\infty\to i$. }

\defn{D4FramedNCDTSeries}{\vardef{hMat}, \vardef{Wp},\vardef{Wm}, \vardef{i}, \vardef{j}, \vardef{k}, \vardef{Nmax}}{constructs the generating function of the D4 framed (motivic) refined NCDT invariants for the quiver with height matrix $\var{hMat}$ and superpotential $\var{Wp}-\var{Wm}$ and for the framing defined by the arrow $\Phi_{i,j}^k$, for dimension vectors with height up to $\var{NMax}$. The refinement here is motivic/cohomological. If the arrow $\Phi_{i,j}^k$ does not correspond to any non-compact 4-cycle and cannot be used as a D4 framing, a message will say it.}

\defn{D6FramedNCDTSeries}{\vardef{hMat}, \vardef{Wp},\vardef{Wm}, \vardef{i}, \vardef{Nmax}, \vardef{weight}}{constructs the generating function of the D6 framed (motivic) refined NCDT invariants for the quiver with height matrix $\var{hMat}$ and superpotential $\var{Wp}-\var{Wm}$ and for the framing defined by the node $i$, for dimension vectors with height up to $\var{NMax}$. The refinement here is motivic/cohomological. $\var{weight}$ is an optional parameter whose value should not affect the result. }
...
\defn{D4Framing}{\vardef{hMat},\vardef{i},\vardef{j},\vardef{k}}{constructs the framing data \var{fMat} for a non-compact D4-brane associated to the $k$-th arrow $i\to j$,  with one arrow $q_i:\infty\to i$,
one arrow $p_j:j \to \infty$ and superpotential $W=W_0+ p_j \Phi_{ij}^k q_i$.} 

\defn{D4FramingDictionary}{\vardef{hMat},\vardef{Wp},\vardef{Wm}}{given a quiver with height matrix $\var{hMat}$ and superpotential $\var{Wp}-\var{Wm}$, constructs a list with as many entries as there are non-compact 4-cycles in the associated variety, such that the $i^{\text{th}}$ element of the list is the list of arrows in the quiver whose associated D4 framing is the framing by the non-compact 4-cycle associated to the $i^{\text{th}}$ corner of the toric diagram. The ordering of the corners is given by applying successively \fun{DualFan} and \fun{ExternalPoints}.} 
...
\defn{CrystalWeight}{\vardef{hMat}, \vardef{fMat}, \vardef{actionWeight}, \vardef{Crys}}
{computes the power of $y$ associated to the molten crystal $\var{Crys}$, using the $\mathbb{C}^\times$ action specified by the weight \var{actionWeight}. The $\mathbb{C}^\times$ action preserves the superpotential if \var{actionWeight} doesn't have a term in $h_3$.}

\defn{DirectedSign}{\vardef{mu}, \vardef{lambda}}
{computes the sign of $\langle \mu, \lambda\rangle$, potentially null, as defined by \fun{WeightScalarProduct}. It is used by \fun{CrystalWeight}.}

\defn{WeightScalarProduct}{\vardef{mu}, \vardef{lambda}}
{computes the scalar product $\langle \mu, \lambda\rangle$ of two weight $\mu$ and $\lambda$, that is $\mu_1\lambda_1+\mu_2\lambda_2+\mu_3\lambda_3$ with $\lambda=\lambda_1 h_1 + \lambda_2 h_2 + \lambda_3 h_3$ and $\mu=\mu_1 h_1 + \mu_2 h_2 + \mu_3 h_3$.}

\defn{ShortestCycleWeight}{\vardef{hMat}, \vardef{node}, \vardef{Cut}, \vardef{maxIters}}
{computes the weight of the shortest cycle starting from $\var{node}$ in the quiver with height matrix $\var{hMat}$, optionally cut by the arrows in $\var{Cut}$. The optional parameter $\var{maxIters}$ specify after how many iterations should the algorithm stop, in case it does not converge. By default, it is set to $10$. If there exists multiple equally short cycles with different weights, one weight will be chosen arbitrarily.}

\defn{SideWeight}{\vardef{hMat}, \vardef{Wp}, \vardef{Wm}, \vardef{i}}
{computes the weight associated to the $\var{i}^{\text{th}}$ side of the toric fan, defining the character of the set of zig-zag paths associated to the given side of the toric fan, for the quiver with height matrix $\var{hMat}$ and superpotential $\var{Wp}-\var{Wm}$. The $\var{i}^{\text{th}}$ side is between the $\var{i}$ and $\var{i}+1$ corners, where the ordering of the corners is given by applying successively \fun{DualFan} and \fun{ExternalPoints}.}
...
\defn{PerfectMatchingWeight}{\vardef{Perf},\vardef{hMat}}{computes the dual of the character of the perfect matching $\var{Perf}$ for the quiver with height matrix $\var{hMat}$, used in \fun{DualFan}.}

\defn{DualFan}{\vardef{hMat},\vardef{Wp},\vardef{Wm}}{computes the dual fan of the quiver with height matrix $\var{hMat}$ and superpotential $\var{Wp}-\var{Wm}$ by computing the characters of the perfect matchings. Since this fan is defined up to an invertible linear transformation, the result is non-canonical.}

\defn{ExternalPoints}{\vardef{Fan}}{gives a list of the corners of the fan $\var{Fan}$, ordered by going clockwise through the corners.}

\defn{InternalSidePoints}{\vardef{points},\vardef{i}}{gives the number of different points lying on the $\var{i}^{\text{th}}$ side of the fan defined by the list $\var{points}$, possibly containing some points with multiplicities. The corners are excluded in the count. The $\var{i}^{\text{th}}$ side is the side lying between the $\var{i}$ and $\var{i}+1$ corners, where the order of the corners is given by applying \fun{ExternalPoints} to $\var{points}$.}

\defn{EulerNorm}{\vardef{hMat},\vardef{Nvec}}{computes the Ringel-Tits norm of the dimension
vector $\var{Nvec}$ from the quiver with height matrix $\var{hMat}$}

\defn{DimensionVectors}{\vardef{hMat},\vardef{Cut}}{gives a list of the dimension vectors corresponding to each connected component of the quiver with height matrix $\var{hMat}$ whose arrows in $\var{Cut}$ have been removed. The list is non-canonically ordered such that a dimension vector comes after another one in the list if there is an arrow going from the first one to the second one in the uncut quiver.}
...
\defn{PlotToricFan}{\vardef{Fan}}{produces a 2D plot of the polygon with vertices listed in \var{Fan}.}

\defn{DisplayArrowMatrix}{\vardef{hMat}}{produces the matrix of arrows associated to the quiver with height matrix $\var{hMat}$, as usually defined in graph theory.}
...
\defn{GeneralWeight}{\vardef{hMat},\vardef{weight},\vardef{Nmax}}{tells whether the weight $\var{weight}$ is general or not up to order $\var{Nmax}$, meaning it is not the weight of a path in the quiver with height matrix $\vardef{hMat}$ of length up to $\var{Nmax}$. }
