...
For computing D6 framed unrefined NCDT invariants for the conifold up to order $x^5$, you may use

% There are multiple issues in the code below. 
\mathematica{1.0}{
\{Tiling, Fan, hMat, Wp, Wm, v1, v2\} = BraneTilingsData[[2]];
UnrefinedSeriesFromCrystal[hMat, D6Framing[hMat, 1], 5] }
   {
   $1 + x_1 - 2 y^3 x_1 x_2 - 4 y^5 x_1^2 x_2 - 2 y^5 x_1^3 x_2 +  y^4 x_1 x_2^2 + 8 y^{10} x_1^2 x_2^2 + 14 y^{14} x_1^3 x_2^2 - 4 y^{13} x_1^2 x_2^3$
}
...
Denef equations. Version 7.0 implements formulas from \cite{Descombes:2021snc} to compute directly D4 and D6 framed refined DT invariants from quivers with potential. 
...
\defn{NCDTSeriesFromCrystal}{\vardef{hMat}, \vardef{fMat},\vardef{theta}, \vardef{phi}, \vardef{Nmax}}{constructs the generating function of (localized) refined NCDT invariants for the quiver with height matrix $\var{hMat}$ and framing data $\var{fMat}$ using the refined Quiver Yangian algorithm with $\mathbb{C}^\times_{\theta,\phi}$ action, for dimension
vectors with height up to $\var{NMax}$. The refinement comes from the localization under the $\mathbb C^\times$ action. The $\mathbb C^\times$ action can also be given by a weight depending on $h_1,h_2$ and $h_3$ instead of by two angles $(\theta,\phi)$. $\var{fMat}$ is a list of two elements: the first is
a matrix of weights $h(q_i^\alpha)$  of arrows $q_i^\alpha:\infty\to i$, and the second is  
...
{computes the weight of the shortest cycle starting from $\var{node}$ in the quiver with height matrix $\var{hMat}$, optionally cut by the arrows in $\var{Cut}$. The optional parameter $\vardef{maxIters}$ specify after how many iterations should the algorithm stop, in case it does not converge. By default, it is set to $1000$. If there exists multiple equally short cycles with different weights, one weight will be chosen arbitrarily.}

\defn{SideWeight}{\vardef{hMat}, \vardef{Wp}, \vardef{Wm}, \vardef{z}}
{computes the weight associated to the $\var{z}^{\text{th}}$ side of the toric fan, that is the dual of the character of the zig-zag paths associated to the $\var{z}^{\text{th}}$ side of the toric fan, for the quiver with height matrix $\var{hMat}$ and superpotential $\var{Wp}-\var{Wm}$. The $\var{z}^{\text{th}}$ side is between the $\var{z}$ and $\var{z}+1$ corners, where the ordering of the corners is given by applying successively \fun{DualFan} and \fun{ExternalPoints}.}
...
\defn{ExternalPoints}{\vardef{Fan}}{gives an oriented ordered list of the corners of the fan $\var{Fan}$.}
...
\defn{DimensionVectors}{\vardef{hMat},\vardef{Cut}}{gives an ordered list of the dimension vectors corresponding to each connected component of the quiver with height matrix $\var{hMat}$ whose arrows in $\var{Cut}$ have been removed. A dimension vector comes after another one in the list if it is possible to go from the first one to the second one in the uncut quiver.}
...
\defn{GeneralWeight}{\vardef{hMat},\vardef{weight},\vardef{Nmax}}{tells wether the weight $\var{weight}$ is general or not up to order $\var{Nmax}$, meaning it is not the weight of a path in the quiver with height matrix $\vardef{hMat}$ of length up to $\var{Nmax}$. }
